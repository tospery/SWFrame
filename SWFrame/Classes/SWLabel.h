...
/**
 `SWLabel` is a drop-in replacement for `UILabel` that supports `NSAttributedString`, as well as automatically-detected and manually-added links to URLs, addresses, phone numbers, and dates.
 
 ## Differences Between `SWLabel` and `UILabel`
 
 For the most part, `SWLabel` behaves just like `UILabel`. The following are notable exceptions, in which `SWLabel` may act differently:
 ...
 `SWLabel`, like `UILabel`, conforms to `NSCoding`. However, if the build target is set to less than iOS 6.0, `linkAttributes` and `activeLinkAttributes` will not be encoded or decoded. This is due to an runtime exception thrown when attempting to copy non-object CoreText values in dictionaries.
 ...
 */
@interface SWLabel : UILabel <SWLabel, UIGestureRecognizerDelegate>
...
/**
 ...
 @discussion A `SWLabel` delegate responds to messages sent by tapping on links in the label. You can use the delegate to respond to links referencing a URL, address, phone number, date, or date with a specified time zone and duration.
 */
@property (nonatomic, unsafe_unretained) IBOutlet id <SWLabelDelegate> delegate;
...
/**
 The vertical text alignment for the label, for when the frame size is greater than the text rect size. The vertical alignment is `SWLabelVerticalAlignmentCenter` by default.
 */
@property (nonatomic, assign) SWLabelVerticalAlignment verticalAlignment;
...
/**
 Adds a link. You can customize an individual link's appearance and accessibility value by creating your own @c SWLabelLink and passing it to this method. The other methods for adding links will use the label's default attributes.
 
 @warning Modifying the link's attribute dictionaries must be done before calling this method.
 
 @param link A @c SWLabelLink object.
 */
...
/**
 Returns the @c SWLabelLink at the give point if it exists.
...
/**
 The `SWLabelDelegate` protocol defines the messages sent to an attributed label delegate when links are tapped. All of the methods of this protocol are optional.
 */
@protocol SWLabelDelegate <NSObject>
 ...
 */
- (void)attributedLabel:(SWLabel *)label
...
/**
 ...
 @discussion This method is called if no other delegate method was called, which can occur by either now implementing the method in `SWLabelDelegate` corresponding to a particular link, or the link was added by passing an instance of a custom `NSTextCheckingResult` subclass into `-addLinkWithTextCheckingResult:`.
...
@interface SWLabelLink : NSObject <NSCoding>

typedef void (^SWFLabelLinkBlock) (SWLabel *, SWLabelLink *);
...
/**
 ...
 @c SWLabelDelegate tap methods, which will not be called for this link.
 */
...
/**
 ...
 @c SWLabelDelegate long press methods, which will not be called for this link.
 */
