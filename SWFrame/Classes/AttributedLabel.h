...
//  iOSFrame
//
//  Created by 杨建祥 on 2020/8/23.
//

#import <UIKit/UIKit.h>
#import <CoreText/CoreText.h>

@class AttributedLabelLink;

/**
 Vertical alignment for text in a label whose bounds are larger than its text bounds
 */
typedef NS_ENUM(NSInteger, AttributedLabelVerticalAlignment) {
    AttributedLabelVerticalAlignmentCenter   = 0,
    AttributedLabelVerticalAlignmentTop      = 1,
    AttributedLabelVerticalAlignmentBottom   = 2,
};
...
@protocol AttributedLabel <NSObject>
...
/**
 `AttributedLabel` is a drop-in replacement for `UILabel` that supports `NSAttributedString`, as well as automatically-detected and manually-added links to URLs, addresses, phone numbers, and dates.
 
 ## Differences Between `AttributedLabel` and `UILabel`
 
 For the most part, `AttributedLabel` behaves just like `UILabel`. The following are notable exceptions, in which `AttributedLabel` may act differently:
 ...
 `AttributedLabel`, like `UILabel`, conforms to `NSCoding`. However, if the build target is set to less than iOS 6.0, `linkAttributes` and `activeLinkAttributes` will not be encoded or decoded. This is due to an runtime exception thrown when attempting to copy non-object CoreText values in dictionaries.
...
/**
 ...
 @discussion A `AttributedLabel` delegate responds to messages sent by tapping on links in the label. You can use the delegate to respond to links referencing a URL, address, phone number, date, or date with a specified time zone and duration.
 */
@property (nonatomic, unsafe_unretained) IBOutlet id <AttributedLabelDelegate> delegate;
...
/**
 The vertical text alignment for the label, for when the frame size is greater than the text rect size. The vertical alignment is `AttributedLabelVerticalAlignmentCenter` by default.
 */
@property (nonatomic, assign) AttributedLabelVerticalAlignment verticalAlignment;
...
/**
 Adds a link. You can customize an individual link's appearance and accessibility value by creating your own @c AttributedLabelLink and passing it to this method. The other methods for adding links will use the label's default attributes.
 
 @warning Modifying the link's attribute dictionaries must be done before calling this method.
 
 @param link A @c AttributedLabelLink object.
 */
- (void)addLink:(AttributedLabelLink *)link;
 ...
 */
- (AttributedLabelLink *)addLinkWithTextCheckingResult:(NSTextCheckingResult *)result;
...
/**
 ...
 @return The newly added link object.
 */
- (AttributedLabelLink *)addLinkToURL:(NSURL *)url
...
/**
 ...
 @return The newly added link object.
 */
- (AttributedLabelLink *)addLinkToAddress:(NSDictionary *)addressComponents
                                   withRange:(NSRange)range;
...
/**
 ...
 @return The newly added link object.
 */
- (AttributedLabelLink *)addLinkToPhoneNumber:(NSString *)phoneNumber
                                       withRange:(NSRange)range;
...
/**
 ...
 @return The newly added link object.
 */
- (AttributedLabelLink *)addLinkToDate:(NSDate *)date
                                withRange:(NSRange)range;
...
/**
 ...
 @return The newly added link object.
 */
- (AttributedLabelLink *)addLinkToTransitInformation:(NSDictionary *)components
                                              withRange:(NSRange)range;
...
/**
 Returns the @c AttributedLabelLink at the give point if it exists.
 
 @discussion This can be used together with @c UIViewControllerPreviewingDelegate to peek into links.
 
 @param point The point inside the label.
 */
- (AttributedLabelLink *)linkAtPoint:(CGPoint)point;

@end

/**
 The `AttributedLabelDelegate` protocol defines the messages sent to an attributed label delegate when links are tapped. All of the methods of this protocol are optional.
 */
 ...
 */
- (void)attributedLabel:(AttributedLabel *)label
...
/**
 ...
 @discussion This method is called if no other delegate method was called, which can occur by either now implementing the method in `AttributedLabelDelegate` corresponding to a particular link, or the link was added by passing an instance of a custom `NSTextCheckingResult` subclass into `-addLinkWithTextCheckingResult:`.
...
@interface AttributedLabelLink : NSObject <NSCoding>

typedef void (^AttributedLabelLinkBlock) (AttributedLabel *, AttributedLabelLink *);
...
/**
 ...
 @c AttributedLabelDelegate tap methods, which will not be called for this link.
 */
@property (nonatomic, copy) AttributedLabelLinkBlock linkTapBlock;

/**
 A block called when this link is long-pressed.
 If non-nil, long pressing on this link will call this block instead of the
 @c AttributedLabelDelegate long press methods, which will not be called for this link.
 */
@property (nonatomic, copy) AttributedLabelLinkBlock linkLongPressBlock;
...
/**
 ...
 @return The initialized link object.
 */
- (instancetype)initWithAttributesFromLabel:(AttributedLabel*)label
                         textCheckingResult:(NSTextCheckingResult *)result;
