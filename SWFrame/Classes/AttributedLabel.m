...
@property (readwrite, nonatomic, strong) AttributedLabelLink *activeLink;
...
- (void)addLinks:(NSArray *)links {
    ...
    for (AttributedLabelLink *link in links) {
        if (link.attributes) {
                                  ...
                                  attributes:(NSDictionary *)attributes
{
    ...
    for (NSTextCheckingResult *result in results) {
        ...
        AttributedLabelLink *link = [[AttributedLabelLink alloc] initWithAttributes:attributes
...
- (AttributedLabelLink *)linkAtPoint:(CGPoint)point {
    ...
    AttributedLabelLink *result = [self linkAtCharacterIndex:[self characterIndexAtPoint:point]];
...
- (AttributedLabelLink *)linkAtRadius:(const CGFloat)radius aroundPoint:(CGPoint)point {
...
- (AttributedLabelLink *)linkAtCharacterIndex:(CFIndex)idx {
...
- (void)setActiveLink:(AttributedLabelLink *)activeLink {
     ...
     limitedToNumberOfLines:(NSInteger)numberOfLines
{
    ...
    if (textSize.height < bounds.size.height) {
        ...
        switch (self.verticalAlignment) {
            case AttributedLabelVerticalAlignmentCenter:
                yOffset = CGFloat_floor((bounds.size.height - textSize.height) / 2.0f);
                break;
            case AttributedLabelVerticalAlignmentBottom:
                yOffset = bounds.size.height - textSize.height;
                break;
            case AttributedLabelVerticalAlignmentTop:
            default:
                break;
        }
...
- (NSArray *)accessibilityElements {
    if (!_accessibilityElements) {
        @synchronized(self) {
            ...
            for (AttributedLabelLink *link in self.linkModels) {
...
- (void)longPressGestureDidFire:(UILongPressGestureRecognizer *)sender {
    switch (sender.state) {
        case UIGestureRecognizerStateBegan: {
            ...
            AttributedLabelLink *link = [self linkAtPoint:touchPoint];
...
#pragma mark - AttributedLabelLink

@implementation AttributedLabelLink
