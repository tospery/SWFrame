 ...
 */
- (nullable id)swf_performSelectorToSuperclass:(SEL)aSelector;
...
/**
 ...
 @link http://stackoverflow.com/questions/14635024/using-objc-msgsendsuper-to-invoke-a-class-method @/link
 */
- (nullable id)swf_performSelectorToSuperclass:(SEL)aSelector withObject:(nullable id)object;
 ...
 *  @code
 *  CGFloat alpha;
 *  [view swf_performSelector:@selector(alpha) withPrimitiveReturnValue:&alpha];
 *  @endcode
 */
- (void)swf_performSelector:(SEL)selector withPrimitiveReturnValue:(nullable void *)returnValue;
 ...
 *  [control swf_performSelector:@selector(addTarget:action:forControlEvents:) withArguments:&target, &action, &events, nil];
 *  @endcode
 */
- (nullable id)swf_performSelector:(SEL)selector withArguments:(nullable void *)firstArgument, ...;
 ...
 *  UIEvent *event = xxx;
 *  BOOL isInside;
 *  [view swf_performSelector:@selector(pointInside:withEvent:) withPrimitiveReturnValue:&isInside arguments:&point, &event, nil];
 *  @endcode
 */
- (void)swf_performSelector:(SEL)selector withPrimitiveReturnValue:(nullable void *)returnValue arguments:(nullable void *)firstArgument, ...;
...
/**
 ...
 @param block  用于遍历的 block
 */
+ (void)swf_enumrateIvarsOfClass:(Class)aClass includingInherited:(BOOL)includingInherited usingBlock:(void (^)(Ivar ivar, NSString *ivarDescription))block;

/**
 使用 block 遍历指定 class 的所有属性，不包含 superclasses 里定义的 property
 
 @param block 用于遍历的 block，如果要获取 property 的信息，推荐用 SWFPropertyDescriptor。
 */
...
/**
 ...
 @param block 用于遍历的 block，如果要获取 property 的信息，推荐用 SWFPropertyDescriptor。
 @see https://developer.apple.com/library/archive/documentation/Cocoa/Conceptual/ObjCSWFRuntimeGuide/Articles/ocrtPropertyIntrospection.html#//apple_ref/doc/uid/TP40008048-CH101-SW1
 */
+ (void)swf_enumratePropertiesOfClass:(Class)aClass includingInherited:(BOOL)includingInherited usingBlock:(void (^)(objc_property_t property, NSString *propertyName))block;

/**
 使用 block 遍历当前实例的所有方法，不包含 superclasses 里定义的 method
 */
- (void)swf_enumrateInstanceMethodsUsingBlock:(void (^)(Method method, SEL selector))block;
...
/**
 ...
 @param block    用于遍历的 block
 */
+ (void)swf_enumrateInstanceMethodsOfClass:(Class)aClass includingInherited:(BOOL)includingInherited usingBlock:(void (^)(Method method, SEL selector))block;
 ...
 */
+ (void)swf_enumerateProtocolMethods:(Protocol *)protocol usingBlock:(void (^)(SEL selector))block;

@end

@interface NSObject (Ex_KeyValueCoding)
...
/**
 ...
 @note SWFrame 提供2种方式兼容系统的 access prohibited 异常：
 1. 通过将配置表的 IgnoreKVCAccessProhibited 置为 YES 来全局屏蔽系统的异常警告，代码中依然正常使用系统的 valueForKey:、setValue:forKey:，当开启后再遇到 access prohibited 异常时，将会用 SWWarnLog 来提醒，不再中断 App 的运行，这是首选推荐方案。
 2. 使用 swf_valueForKey:、swf_setValue:forKey: 代替系统的 valueForKey:、setValue:forKey:，适用于不希望全局屏蔽，只针对某个局部代码自己处理的场景。
 
 @param key ivar 属性名，支持下划线或不带下划线
 @return key 对应的 value，如果该 key 原本是非对象的值，会被用 NSNumber、NSValue 包裹后返回
 */
- (nullable id)swf_valueForKey:(NSString *)key;
...
/**
 ...
 @return key 对应的 value，如果该 key 原本是非对象的值，会被用 NSNumber、NSValue 包裹后返回
 */
- (void)swf_setValue:(nullable id)value forKey:(NSString *)key;
...
@interface NSObject (Ex_DataBind)
...
/**
 ...
 [cell swf_bindObject:indexPath forKey:@"indexPath"];
 }
 
 - (void)didTapButton:(UIButton *)button {
 // 2）在这里取出被点击的 button 的 indexPath 对象
 NSIndexPath *indexPathTapped = [button swf_getBoundObjectForKey:@"indexPath"];
 }
 @endcode
 */
- (void)swf_bindObject:(nullable id)object forKey:(NSString *)key;

/**
 给对象绑定上另一个对象以供后续取出使用，但相比于 swf_bindObject:forKey:，该方法不会 strong 强引用传入的 object
 */
- (void)swf_bindObjectWeakly:(nullable id)object forKey:(NSString *)key;

/**
 取出之前使用 bind 方法绑定的对象
 */
- (nullable id)swf_getBoundObjectForKey:(NSString *)key;
...
@property(nonatomic, assign) BOOL swf_shouldIgnoreUIKVCAccessProhibited;
@end
