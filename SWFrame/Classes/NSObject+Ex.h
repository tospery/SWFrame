 ...
 */
- (nullable id)sf_performSelectorToSuperclass:(SEL)aSelector;
...
/**
 ...
 @link http://stackoverflow.com/questions/14635024/using-objc-msgsendsuper-to-invoke-a-class-method @/link
 */
- (nullable id)sf_performSelectorToSuperclass:(SEL)aSelector withObject:(nullable id)object;
 ...
 *  @code
 *  CGFloat alpha;
 *  [view sf_performSelector:@selector(alpha) withPrimitiveReturnValue:&alpha];
 *  @endcode
 */
- (void)sf_performSelector:(SEL)selector withPrimitiveReturnValue:(nullable void *)returnValue;
 ...
 *  [control sf_performSelector:@selector(addTarget:action:forControlEvents:) withArguments:&target, &action, &events, nil];
 *  @endcode
 */
- (nullable id)sf_performSelector:(SEL)selector withArguments:(nullable void *)firstArgument, ...;
 ...
 *  UIEvent *event = xxx;
 *  BOOL isInside;
 *  [view sf_performSelector:@selector(pointInside:withEvent:) withPrimitiveReturnValue:&isInside arguments:&point, &event, nil];
 *  @endcode
 */
- (void)sf_performSelector:(SEL)selector withPrimitiveReturnValue:(nullable void *)returnValue arguments:(nullable void *)firstArgument, ...;
...
/**
 ...
 @param block  用于遍历的 block
 */
+ (void)sf_enumrateIvarsOfClass:(Class)aClass includingInherited:(BOOL)includingInherited usingBlock:(void (^)(Ivar ivar, NSString *ivarDescription))block;

/**
 使用 block 遍历指定 class 的所有属性，不包含 superclasses 里定义的 property
 
 @param block 用于遍历的 block，如果要获取 property 的信息，推荐用 SWPropertyDescriptor。
 */
 ...
 */
+ (void)sf_enumratePropertiesOfClass:(Class)aClass includingInherited:(BOOL)includingInherited usingBlock:(void (^)(objc_property_t property, NSString *propertyName))block;

/**
 使用 block 遍历当前实例的所有方法，不包含 superclasses 里定义的 method
 */
- (void)sf_enumrateInstanceMethodsUsingBlock:(void (^)(Method method, SEL selector))block;
...
/**
 ...
 @param block    用于遍历的 block
 */
+ (void)sf_enumrateInstanceMethodsOfClass:(Class)aClass includingInherited:(BOOL)includingInherited usingBlock:(void (^)(Method method, SEL selector))block;
 ...
 */
+ (void)sf_enumerateProtocolMethods:(Protocol *)protocol usingBlock:(void (^)(SEL selector))block;
...
/**
 ...
 2. 使用 sf_valueForKey:、sf_setValue:forKey: 代替系统的 valueForKey:、setValue:forKey:，适用于不希望全局屏蔽，只针对某个局部代码自己处理的场景。
 
 @param key ivar 属性名，支持下划线或不带下划线
 @return key 对应的 value，如果该 key 原本是非对象的值，会被用 NSNumber、NSValue 包裹后返回
 */
- (nullable id)sf_valueForKey:(NSString *)key;
...
/**
 ...
 @return key 对应的 value，如果该 key 原本是非对象的值，会被用 NSNumber、NSValue 包裹后返回
 */
- (void)sf_setValue:(nullable id)value forKey:(NSString *)key;
...
/**
 ...
 [cell sf_bindObject:indexPath forKey:@"indexPath"];
 }
 
 - (void)didTapButton:(UIButton *)button {
 // 2）在这里取出被点击的 button 的 indexPath 对象
 NSIndexPath *indexPathTapped = [button sf_getBoundObjectForKey:@"indexPath"];
 }
 @endcode
 */
- (void)sf_bindObject:(nullable id)object forKey:(NSString *)key;

/**
 给对象绑定上另一个对象以供后续取出使用，但相比于 sf_bindObject:forKey:，该方法不会 strong 强引用传入的 object
 */
- (void)sf_bindObjectWeakly:(nullable id)object forKey:(NSString *)key;

/**
 取出之前使用 bind 方法绑定的对象
 */
- (nullable id)sf_getBoundObjectForKey:(NSString *)key;
...
@property(nonatomic, assign) BOOL sf_shouldIgnoreUIKVCAccessProhibited;
@end
