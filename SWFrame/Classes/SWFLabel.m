...
const NSTextAlignment SWFTextAlignmentRight = NSTextAlignmentRight;
const NSTextAlignment SWFTextAlignmentJustified = NSTextAlignmentJustified;
const NSTextAlignment SWFTextAlignmentNatural = NSTextAlignmentNatural;

const NSLineBreakMode SWFLineBreakByWordWrapping = NSLineBreakByWordWrapping;
const NSLineBreakMode SWFLineBreakByCharWrapping = NSLineBreakByCharWrapping;
const NSLineBreakMode SWFLineBreakByClipping = NSLineBreakByClipping;
const NSLineBreakMode SWFLineBreakByTruncatingHead = NSLineBreakByTruncatingHead;
const NSLineBreakMode SWFLineBreakByTruncatingMiddle = NSLineBreakByTruncatingMiddle;
const NSLineBreakMode SWFLineBreakByTruncatingTail = NSLineBreakByTruncatingTail;

typedef NSTextAlignment SWFTextAlignment;
typedef NSLineBreakMode SWFLineBreakMode;


static inline CTTextAlignment CTTextAlignmentFromSWFTextAlignment(SWFTextAlignment alignment) {
    switch (alignment) {
...
static inline CTLineBreakMode CTLineBreakModeFromSWFLineBreakMode(SWFLineBreakMode lineBreakMode) {
...
static inline CGFloat SWFFlushFactorForTextAlignment(NSTextAlignment textAlignment) {
    switch (textAlignment) {
        case SWFTextAlignmentCenter:
            return 0.5f;
        case SWFTextAlignmentRight:
            return 1.0f;
        case SWFTextAlignmentLeft:
        default:
            return 0.0f;
    }
}
...
static inline NSDictionary * NSAttributedStringAttributesFromLabel(SWFLabel *label) {
    ...
    if ([NSMutableParagraphStyle class]) {
        ...
        CTTextAlignment alignment = CTTextAlignmentFromSWFTextAlignment(label.textAlignment);
        ...
        if (label.numberOfLines == 1) {
            lineBreakMode = CTLineBreakModeFromSWFLineBreakMode(label.lineBreakMode);
        }
...
static inline CGSize CTFramesetterSuggestFrameSizeForAttributedStringWithConstraints(CTFramesetterRef framesetter, NSAttributedString *attributedString, CGSize size, NSUInteger numberOfLines) {
    ...
    CGSize constraints = CGSizeMake(size.width, SWFFLOAT_MAX);

    if (numberOfLines == 1) {
        // If there is one line, the size that fits is the full width of the line
        constraints = CGSizeMake(SWFFLOAT_MAX, SWFFLOAT_MAX);
    } else if (numberOfLines > 0) {
        // If the line count of the label more than 1, limit the range to size to the number of lines that have been set
        CGMutablePathRef path = CGPathCreateMutable();
        CGPathAddRect(path, NULL, CGRectMake(0.0f, 0.0f, constraints.width, SWFFLOAT_MAX));
...
@interface SWFAccessibilityElement : UIAccessibilityElement
@property (nonatomic, weak) UIView *superview;
@property (nonatomic, assign) CGRect boundingRect;
@end

@implementation SWFAccessibilityElement
...
- (CFIndex)characterIndexAtPoint:(CGPoint)p {
    ...
    for (CFIndex lineIndex = 0; lineIndex < numberOfLines; lineIndex++) {
        ...
        CGFloat flushFactor = SWFFlushFactorForTextAlignment(self.textAlignment);
                ...
                context:(CGContextRef)c
{
    ...
    CFArrayRef lines = CTFrameGetLines(frame);
    NSInteger numberOfLines = self.numberOfLines > 0 ? MIN(self.numberOfLines, CFArrayGetCount(lines)) : CFArrayGetCount(lines);
    BOOL truncateLastLine = (self.lineBreakMode == SWFLineBreakByTruncatingHead || self.lineBreakMode == SWFLineBreakByTruncatingMiddle || self.lineBreakMode == SWFLineBreakByTruncatingTail);
    ...
    for (CFIndex lineIndex = 0; lineIndex < numberOfLines; lineIndex++) {
        ...
        if (lineIndex == numberOfLines - 1 && truncateLastLine) {
            ...
            if (!(lastLineRange.length == 0 && lastLineRange.location == 0) && lastLineRange.location + lastLineRange.length < textRange.location + textRange.length) {
                ...
                SWFLineBreakMode lineBreakMode = self.lineBreakMode;

                // Multiple lines, only use UILineBreakModeTailTruncation
                if (numberOfLines != 1) {
                    lineBreakMode = SWFLineBreakByTruncatingTail;
                }

                switch (lineBreakMode) {
                    case SWFLineBreakByTruncatingHead:
                        truncationType = kCTLineTruncationStart;
                        break;
                    case SWFLineBreakByTruncatingMiddle:
                        truncationType = kCTLineTruncationMiddle;
                        truncationAttributePosition += (lastLineRange.length / 2);
                        break;
                    case SWFLineBreakByTruncatingTail:
                    default:
...
- (void)drawTextInRect:(CGRect)rect {
    ...
    if (self.adjustsFontSizeToFitWidth && self.numberOfLines > 0) {
        ...
        CGSize maxSize = (self.numberOfLines > 1) ? CGSizeMake(SWFFLOAT_MAX, SWFFLOAT_MAX) : CGSizeZero;
        ...
        @synchronized(self) {
            ...
            for (SWFLabelLink *link in self.linkModels) {
                ...
                if (accessibilityLabel) {
                    SWFAccessibilityElement *linkElement = [[SWFAccessibilityElement alloc] initWithAccessibilityContainer:self];
            ...
            SWFAccessibilityElement *baseElement = [[SWFAccessibilityElement alloc] initWithAccessibilityContainer:self];
