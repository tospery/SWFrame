...
@implementation WebViewJSBridgeBase {
...
- (void)flushMessageQueue:(NSString *)messageQueueString{
    ...
    for (WVJSBMessage* message in messages) {
        if (![message isKindOfClass:[WVJSBMessage class]]) {
        ...
        if (responseId) {
            WVJSBResponseCallback responseCallback = _responseCallbacks[responseId];
            responseCallback(message[@"responseData"]);
            [self.responseCallbacks removeObjectForKey:responseId];
        } else {
            WVJSBResponseCallback responseCallback = NULL;
            ...
            if (callbackId) {
                responseCallback = ^(id responseData) {
                    ...
                    WVJSBMessage* msg = @{ @"responseId":callbackId, @"responseData":responseData };
            ...
            WVJSBHandler handler = self.messageHandlers[message[@"handlerName"]];
...
- (void)_queueMessage:(WVJSBMessage*)message {
    if (self.startupMessageQueue) {
...
- (void)_dispatchMessage:(WVJSBMessage*)message {
